...
{
private:
    zong::geometry::Segment _segment2d;           // 该线段，优化的时候可以考虑去掉这个变量
    PointEvent              _startPointEvent;     // 起点事件，用来查找另一点的事件
    zong::geometry::Point   _sweepLineCrossPoint; // 与扫描线的交点，用来排序
    double                  _k;                   // 斜率
...
public:
    ...
    inline zong::geometry::Segment segment() const { return _segment2d; }
    inline PointEvent              startPointEvent() const { return _startPointEvent; }
    inline zong::geometry::Point   sweepLineCrossPoint() const { return _sweepLineCrossPoint; }
    inline bool                    isNoK() const { return _isNoK; }

    inline void setSegmentSecond(zong::geometry::Point const& second) { _segment2d.setSecond(second); }
